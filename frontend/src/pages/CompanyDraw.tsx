...
let windup = new Audio("/sounds/scifi2.wav");
...
export default function CompanyDraw() {
  ...
  const handleStart = () => {
    windup.currentTime = windup.duration - 4.5;
    windup.volume = 0;
    windup.play();
    gsap.to(windup, {
      volume: 1,
    ...
    <div className="flex h-[100vh] w-full items-center justify-center" style={{ opacity: 0 }}>
      <div className="relative flex h-[100vh] w-full flex-col items-center justify-center overflow-hidden bg-background">
        ...
        <div className="relative mt-4">
          <CustomButton className="z-40 relative overflow-hidden" onClick={handleStart}>Start</CustomButton>
          <Mask className="absolute inset-0 z-30 opacity-0 bg-black" ref={maskRef}/>
        </div>
